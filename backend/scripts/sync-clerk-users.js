...
async function main() {
  ...
  for (const local of users) {
    ...
    if (name !== local.name || role !== local.role || avatarUrl !== local.avatarUrl || email !== local.email) {
      await prisma.user.update({ where: { id: local.id }, data: { name, role, avatarUrl, email } });
